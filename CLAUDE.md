...
The doc is a review canvas: newest turn first. Each `<section class="dv-turn" id="tN">` is one conversation turn (currently t9, t8, t7, t6, t5, t4, t3, t2 top-to-bottom; t7 = the winning novice-first how-it-works page, t8 = the rest of the site in daylight, t9 = the same current story re-spoken in the archived 2a quiet-engineering and 2c hybrid dialects, sharing a `tg*` live-trigger demo slice and the old `b*` branding editor), containing `.dv-opt` design options with ids like `4a`, `2a`, `2c`, a `data-screen-label`, a fixed-width `.dv-card`, and a closing `.dv-next` paragraph proposing what to build next. New work goes in as a **new turn section at the top of `<x-dc>`**, with the next turn number.
